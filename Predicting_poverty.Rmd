...
only the first dataset has some missing data. 
...
for (i in seq_along(model_train_hhold_data)) {
  ...
  model_train_hhold_data[[i]][colSums(is.na(model_train_hhold_data[[i]])) > 0]
}
...
temp <- function(df, prcnt, return_train, return_test) {
  
  # this function will only work properly if seed has been set correctly
  set.seed(123)
  
  # sample indices
  sample_size <- floor(prcnt * nrow(df))
  train_ind <- sample(seq_len(nrow(df)), size = sample_size)

  if(return_train==TRUE) {
    train <- subset(x = df[train_ind, ], select = -c(id))
    return(train)
  }
  
  if(return_test==TRUE) {
    test <- subset(x = df[-train_ind, ], select = -c(id))
    return(test)
  }
...
for (i in seq_along(models)) {
  ...
  predictions[[i]] <- models[[i]] %>%
    ...
    # add back id
    bind_cols(
      raw_train_hhold_data[[i]][-train_ind, -raw_train_hhold_data[[i]]$id]) %>%
